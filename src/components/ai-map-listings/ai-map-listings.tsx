...
export type { AIMapListingsProps };
...
export function AIMapListings({
  ...
  // Handle map updates from NLP search
  const handleMapUpdate = useCallback((update: { center?: [number, number]; bounds?: number[][][]; zoom?: number }) => {
    if (!map.current) return;

    if (update.center) {
      map.current.easeTo({
        center: update.center,
        zoom: update.zoom || map.current.getZoom(),
        duration: 1000,
      });
    }
  }, []);
